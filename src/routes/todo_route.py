...
@router.get("/todos",
            ...
            description="Gets all of created tasks from database for a specific user.")
def get_todos(db: Session = Depends(get_db), user=Depends(get_auth_user)):
    return todo_crud.get_all_todos(db=db, user_id=user["id"])


@router.get("/todo/{id}",
            response_model=schemas.Todo,
            description="Gets a task by id.")
def get_todo_by_id(id: int = Path(..., ge=0),
...
def post_todo(todo: schemas.TodoCreate,
              db: Session = Depends(get_db),
              user=Depends(get_auth_user)):
    return todo_crud.create_todo(db=db, request=todo, user_id=user["id"])


@router.put("/todo/{id}", response_model=schemas.Todo, description="Updates a task by id.")
def update_todo(todo: schemas.TodoUpdate,
                id: int = Path(..., ge=0), db: Session = Depends(get_db),
                user=Depends(get_auth_user)):
    return todo_crud.update_todo(db=db, request=todo, id=id)


@router.delete("/todo/{id}", description="Deletes a task by id.")
def delete_todo(id: int = Path(..., ge=0),
                db: Session = Depends(get_db),
                user=Depends(get_auth_user)):
